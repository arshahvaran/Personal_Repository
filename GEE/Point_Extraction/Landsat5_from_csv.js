...
var csvFile = ee.FeatureCollection("projects/ee-arshahvaran/assets/gee_l5");
...
var extractPixelValues = function(feature) {
  ...
  var landsat7SRCollection = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2')
    .filterBounds(point)
  ...
  var landsat7TOACollection = ee.ImageCollection('LANDSAT/LT05/C02/T1')
    .filterBounds(point)
    .filterDate(startDate, endDate)
    .filter(ee.Filter.eq('WRS_PATH', path))
    .filter(ee.Filter.eq('WRS_ROW', row))
...
Export.table.toDrive({
  ...
  description: 'Updated_Landsat5_Pixel_Values',
  fileFormat: 'CSV'
});
